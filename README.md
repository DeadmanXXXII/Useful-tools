🛠️ Useful Pentesting Tools

A comprehensive list of tools categorized by phase of a pentest engagement — from OSINT to clean-up. Continuously evolving and extended.


---

🔍 1. Information Gathering and OSINT

TheHarvester

SpiderFoot

Recon-ng

Maltego (and Maltego CE)

Sherlock

OSINT Framework

Datasploit

Pipl

Shodan (+ CLI)

Censys (+ CLI)

Google Dorks (GHDB)

Whois

Netcraft

EmailHunter (Hunter.io)

Have I Been Pwned

Robtex

Panguin Tool

BuiltWith

VirusTotal

FOCA

Sublist3r

Amass (passive and active)

Wayback Machine / Waybackurls

DNSDumpster

LinkedIn Scraper

ReconDog

S3Scanner

Aquatone

Metagoofil

Findomain

DuckDuckGo Scraper

Scrapy

WPScan

Curl / TorCurl

FOFA

BinaryEdge

GitRob

GitLeaks

Emailrep.io

ExifTool



---

📡 2. Network Scanning

Nmap (and NSE)

Netcat / Ncat

Tcpdump

Wireshark / Tshark

Angry IP Scanner

ZMap

Masscan

Unicornscan

Hping3

Fing

Netdiscover

EtherApe

LanSweeper

Netstat

P0f

NetworkMiner

Netwag

Socat

PacketCapture tools

Ettercap (ARP spoofing)

Xplico

Kismet

Nping

Tcpflow

RustScan



---

🛡️ 3. Vulnerability Scanning

Nessus (+ CLI)

OpenVAS / GVM (+ CLI)

Nikto

Nexpose / Rapid7 InsightVM

Qualys

Arachni

W3AF

Burp Suite Scanner

Retina

Vuls

Clair

Trivy

OWASP Dependency-Check

Prowler

ScoutSuite

CloudSploit

Kube-Bench

Kube-Hunter

Lynis

Gf (Golang Fuzzing)

Vulners

XSSStrike

Gato

WAFW00F

S3Scanner

VIDAR

Scanless

Vulnscan

Nuclei

SSLscan

OpenSSL

LinScan

CloudMapper



---

🌐 4. Web Application Scanning

Burp Suite

OWASP ZAP

W3AF

Arachni

SQLMap

Skipfish

NetSparker (Invicti)

Acunetix

XSSer

WebInspect

Nikto

DirBuster

Dirsearch

Gobuster

WhatWeb

Wappalyzer

OpenBugBounty

Fuzzer

XSSStrike

WAFW00F

TestSSL

OWASP Amass

WebScarab

SpiderPig

FFUF

ParamSpider

Cariddi



---

💥 5. Exploitation

Metasploit Framework

Cobalt Strike

Empire (PowerShell Empire)

Pupy

Koadic

AhMyth

Frida

BeEF

Social-Engineer Toolkit (SET)

Veil-Framework

PayloadsAllTheThings

Armitage

Msfvenom

Netcat / Ncat

ExploitDB

Binwalk

Ettercap

Responder

Setoolkit

Kismet

NTLMRelayx

SMBExec

CrackMapExec

JuicyPotato / BadPotato

Evil-WinRM

Mimikatz

BloodHound

Sliver

Mythic

Chisel

Merlin

Pwncat



---

🔓 6. Post-Exploitation

Mimikatz

BloodHound

PowerSploit

Windows Credential Editor (WCE)

Impacket

CrackMapExec

SharpHound

Evil-WinRM

FuzzBunch

Responder

SMBRelay

Cobalt Strike Beacon

PowerView / PowerUp

Kerberoast

PsExec

Invoke-Phant0m

Seatbelt

LaZagne

SharpUp

Rubeus

Jupyter Notebook for data analysis



---

📈 7. Privilege Escalation

WinPEAS / LinPEAS (PEASS-ng)

PowerSploit (PowerUp)

Kernel exploits (e.g., Dirty COW)

Sudo permissions enumeration

Linux Exploit Suggester

Windows Exploit Suggester

Enum4linux

LinEnum

Sudo-Kill

Privilege Escalation Cheat Sheets

GTFOBins

LOLbas (Living Off The Land Binaries and Scripts)

BeRoot



---

📝 8. Reporting

SecLists

PayloadsAllTheThings

Rockyou.txt

FuzzDB

XSS Cheat Sheet

CVE List

Dradis

Faraday

Serpico

ReportGenerator

Metasploit Pro Reporting

Nexpose Report

PhishTool

Safety

CherryTree

OWASP ZAP / Burp Suite report features

Threat Dragon

Scribe

ReportLab

Trello / Jira

Obsidian

Hacktricks



---

🧹 9. Post-Test Clean-Up

Manual verification (remove shells, users, files)

CCleaner / BleachBit

DBAN / Disk wipe tools

Logrotate / LogCleaner

SDelete / Eraser (Sysinternals)

Auditpol (Windows)

Auditd tools (Linux)

Final system verification scripts

Log analysis tools

System recovery tools

Post-report cleanup



---

✨ Wordlists & Payloads

SecLists

PayloadsAllTheThings

FuzzDB

Rockyou.txt



---

💬 Contributing

Pull requests and suggestions welcome! This list is intended to grow with community input.


---
